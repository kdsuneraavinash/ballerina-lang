...
public class ParameterNameCache {
    ...
    public static ParameterNameCache fromConfig(QuoterConfig config) {
        ...
        try (InputStream inputStream = classLoader.getResourceAsStream(jsonFile)) {
            if (inputStream == null) {
                String errorMessage = "Parameter cache map file not found: " + EXTERNAL_NODE_CHILDREN_JSON;
            ...
            e.printStackTrace();
            throw new QuoterException("Parameter cache file reading failed: " + e.getMessage());
        }
    }
